    ...
    const renderIcon = (iconType, colorClass) => {
        ...
        return (
            <div className={`${colorClass} p-3 rounded-full`}>
                {iconMap[iconType] || iconMap.users}
            </div>
        );
    };
            ...
            {statsData.map((stat, index) => (
                <div key={index} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
                    <div className="flex items-center justify-between">
                        ...
                        {renderIcon(stat.icon, stat.iconBgColor)}
                    </div>
                    {stat.trend && (
                        <div className="mt-3 flex items-center">
                            <span className={`text-xs ${stat.trend.isPositive ? 'text-green-600' : 'text-red-600'} flex items-center`}>
